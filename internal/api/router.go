...
func NewRouter(h *Handler, staticFS embed.FS, debugMode bool, authMiddleware *auth.Middleware) *http.ServeMux {
	...
	// Login route
	mux.Handle("GET /auth/login", wrap(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte("Shrinkray login - No UI available"))
	})))
	...
	if err != nil {
		...
		// Serve login page
		mux.Handle("GET /auth/login", wrap(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			content, err := fs.ReadFile(staticSubFS, "login.html")
			if err != nil {
				http.Error(w, "Not found", http.StatusNotFound)
				return
			}
